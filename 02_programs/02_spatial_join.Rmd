...
ar_oz_bd$centroid <- st_centroid(ar_oz_bd$geometry)

ggplot() +
  geom_sf(data = oz_bound, color = "blue") +
  plot(ar_oz_bd$geometry, axes = TRUE)
  plot(st_centroid(ar_oz_bd$geometry), add = TRUE, pch = 3, col = 'red')
...
distances <- ar_oz_bd %>% 
  st_cast("POINT") %>% 
  st_distance(st_centroid(ar_oz_bd$geometry))
