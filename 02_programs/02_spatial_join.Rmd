...
lion_sf_combined <- lion_bf_lines %>%
  st_buffer(singleSide = T, dist = 10) %>%
  as.data.frame() %>% 
  left_join(lion_bf_lines %>%
              st_buffer(singleSide = T, dist = -10),
            by="bf_id") %>%
  rowwise() %>% 
  mutate(shape = list(st_geometrycollection(c(shape.x,shape.y)))) %>%
...
```{r}
lion_sf_combined <- lion_sf_combined %>% 
  ungroup() %>% 
  st_set_crs(2263) %>%
  st_make_valid() %>% 
  group_by(bf_id) %>% 
...
# spatial joins
ar_oz <- ar_sf %>%
  st_join(lion_sf_combined, left = TRUE)
# %>% 
