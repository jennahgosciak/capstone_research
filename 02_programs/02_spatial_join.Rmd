...
```{r}
# load lion
lion <- st_read("../00_data/nyclion_22c/lion/lion.gdb", layer="lion")
lion_bf <- lion %>% 
  select(SegmentID, LBlockFaceID, RBlockFaceID) %>% 
  rename_all(~str_to_lower(.)) %>% 
  mutate(bf_id = str_c(lblockfaceid, rblockfaceid)) %>% 
  filter(str_trim(bf_id) != "") %>% 
  filter(!is.na(lblockfaceid) & !is.na(rblockfaceid)) %>% 
  group_by(segmentid) %>% 
  filter(row_number() == 1) %>% 
  ungroup()

lion <- st_cast(lion, "MULTILINESTRING")
lion <- st_make_valid(lion)
lion_bf_lines <- lion %>%
  rename_all(~str_to_lower(.)) %>% 
  mutate(bf_id = str_c(lblockfaceid, rblockfaceid)) %>% 
  filter(!is.na(lblockfaceid) & !is.na(rblockfaceid)) %>% 
...
lion_bf_lines %>% 
  st_write("../00_data/lion_bf_lines.geojson", dsn_delete = T)
```
...
# plotting bound box
bound_box <- ar_sf %>% 
  filter(arrest_precinct == 14, arrest_boro == "M") %>% 
  arrange(x_coord_cd) %>% 
  head(1000) %>% 
  st_bbox()

lion_crop <- lion_bf_lines %>% 
  filter(str_trim(bf_id) != "") %>% 
  st_crop(bound_box) %>% 
  st_transform(2263) %>% 
  st_buffer(10)
...
ar_sf_crop <- ar_sf  %>% 
  st_crop(bound_box) %>% 
  st_join(lion_crop, left = T)

ar_sf_crop %>% 
  ggplot() +
  geom_sf(data = lion_crop, color = 'black',
          linewidth = 0.7,
          aes(fill = bf_id)) +
  geom_sf() +
  theme_void() +
...
ar_oz %>%
  as.data.frame() %>% 
  mutate(isna_bf_id = if_else(is.na(bf_id), 1, 0)) %>% 
  summarize(missing_bf_id = mean(isna_bf_id))

ar_oz %>% 
  as.data.frame() %>% 
  group_by(arrest_key, arrest_date) %>% 
  summarize(n = n()) %>% 
  ungroup() %>% 
