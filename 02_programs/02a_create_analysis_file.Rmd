...
ar_oz2 <- ar_oz2 %>%
  ...
  pivot_wider(
    id_cols = c("GEOID_b", "GEOID", "post","distance_boundary"), names_from = "quarter", values_from =
      ends_with("_ar")
  ) %>%
...
cp_oz2 <- cp_oz2 %>%
  ...
  pivot_wider(id_cols = c("GEOID_b", "GEOID", "post", "distance_boundary"),
              names_from = "quarter", values_from = ends_with("_cp")) %>%
  left_join(acs_oz_vars, by = "GEOID") %>%
  mutate(across(starts_with("violent_ellen"), ~if_else(is.na(.), 0, .))) %>%
  mutate(across(starts_with("property_ellen"), ~if_else(is.na(.), 0, .)))
```
