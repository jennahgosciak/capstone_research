...
# withOUT remerging ACS
analysis_file <- full_join(cp_oz3, ar_oz3 %>%
  select(
    GEOID_b, quarter, violent_ellen_ar, violent_ellen_winsorize_ar,
    property_ellen_ar, property_ellen_winsorize_ar,
    distance_boundary_ar
  ),
by = c("GEOID_b", "quarter")
) %>%
  arrange(quarter) %>%
  pivot_wider(
    names_from = quarter,
    values_from = c(
      violent_ellen_ar, violent_ellen_winsorize_ar, property_ellen_ar,
...
analysis_file2 <- full_join(cp_oz3, ar_oz3 %>%
  select(
    GEOID_b, quarter, violent_ellen_ar, violent_ellen_winsorize_ar,
    property_ellen_ar, property_ellen_winsorize_ar,
    distance_boundary_ar
...
by = c("GEOID_b", "quarter")
  ...
  # Not sure if I did that right? Or in the right place? Doesn't seem to do anything?
  mutate(across(starts_with("violent_ellen"), ~ if_else(is.na(.), 0, .))) %>%
  mutate(across(starts_with("property_ellen"), ~ if_else(is.na(.), 0, .)))
```
