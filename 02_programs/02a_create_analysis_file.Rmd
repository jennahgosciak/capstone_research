...
centroid_distances  <- st_read("../03_output/centroid_distances.geojson") %>% 
  ...
  # filtering out ineligible census blocks
...
ar_oz2 <- ar_oz %>%
  ...
  group_by(
    GEOID_b, GEOID, quarter, DESIGNATED, pct_white, pct_higher_ed, pct_rent, pct_native_hc_covered,
...
analysis_file <- full_join(ar_oz3, cp_oz3,
  ...
  mutate(centroid_distance_boundary = if_else(DESIGNATED == T, -1*centroid_distance_boundary,
