...
ar_oz2 <- ar_oz %>%
  mutate(quarter = case_when(month(arrest_date) %in% c(5, 6, 7) & year(arrest_date) == 2018 ~ 1,
...
cp_oz2 <- cp_oz %>%
  mutate(quarter = case_when(month(cmplnt_fr_dt) %in% c(5, 6, 7) & year(cmplnt_fr_dt) == 2018 ~ 1,
