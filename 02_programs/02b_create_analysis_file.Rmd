...
analysis_file_fe <- full_join(ar_oz3, cp_oz3,
  ...
  mutate(
    quarter2 = case_when(quarter %in% c(-4, 0, 4) ~ 1,
                        quarter %in% c(-3, 1, 5) ~ 2,
                        quarter %in% c(-2, 2, 6) ~ 3,
                        quarter %in% c(-1, 3, 7) ~ 4)
  ) %>% 
  filter(!is.na(quarter2))

grid <- expand.grid(GEOID_b = unique(analysis_file$GEOID_b), 
                    year = c("year-1", "year0", "year1"),
            quarter2 = c(1:4)) 
```

```{r}
analysis_file_fe <- analysis_file_fe %>%
  select(-quarter, GEOID) %>% 
  #filter(!is.na(quarter2) & !is.na(GEOID)) %>%
  full_join(grid, by = c("GEOID_b", "year", "quarter2"))  %>% 
  pivot_wider(
    id_cols = c(GEOID_b, quarter2),
    names_from = year,
    values_from = c(starts_with("violent_ellen"), starts_with("property_ellen"))
  ) %>% 
  left_join(distinct(analysis_file, GEOID_b, GEOID), by = "GEOID_b")%>%
  ...
  filter(!is.na(quarter2) & !is.na(GEOID_b))
