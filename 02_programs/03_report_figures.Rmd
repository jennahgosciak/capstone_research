...
acs_oz %>%
  ...
  summarize(across(c(names(acs_vars),constructs, urbanvars), mean, na.rm = T)) %>%
  pivot_longer(cols = c(names(acs_vars), constructs, urbanvars)) %>%
  mutate(
    value = round(value, 2),
...
acs_oz %>%
  ...
  pivot_wider(id_cols = "name", names_from = "designated_desc", values_from = "value") %>%
  select(name, designated, eligible) %>% 
  summarize(designated = max(designated),
...
), ~ test_baseline_significance(., data = cb_adj_acs_tract %>%
...
ar_oz_plots %>%
  ...
  labs(
    ...
    x = "Distance from the boundary (feet)"
  )
...
cp_oz_plots %>%
  ...
  facet_wrap(~post, nrow = 1, scales = "free_y") +
  labs(
    y = "Number of property crime complaints",
    x = "Distance from the boundary (feet)"
  )
