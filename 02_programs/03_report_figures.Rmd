...
ar_oz <- ar_oz %>%
  mutate(arrest_month_yr = as.Date(str_c(month(arrest_date), 1, year(arrest_date), sep="-"),
                                   "%m-%d-%Y")) %>% 
  mutate(year = year(arrest_month_yr))

cp_oz <- cp_oz %>% 
  mutate(cp_month_yr = as.Date(str_c(month(cmplnt_fr_dt), 1, year(cmplnt_fr_dt), sep="-"),
                               "%m-%d-%Y")) %>% 
  mutate(year = year(cp_month_yr))

st_oz <- st_oz %>% 
  mutate(quarter = case_when(month(occur_date) <= 3 ~ 1,
                             month(occur_date) <= 6 ~ 2,
                             month(occur_date) <= 9 ~ 3,
                             month(occur_date) <= 12 ~ 4),
         st_month_yr = as.Date(str_c(month(occur_date), 1, year(occur_date), sep="-"), 
                               "%m-%d-%Y"),
