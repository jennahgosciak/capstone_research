...
cp_oz_acs <- read_csv("../00_data/cp_oz_acs.csv")
...
outcomes_ar <- c("violent_ellen", "property_ellen")

create_plots <- function(df, outcomes, labs, fname) {
  # unadjusted plot
  df %>%
    ungroup() %>% 
    pivot_longer(c(violent_ellen, property_ellen))  %>% 
    mutate(name = factor(labs_ar[match(name, outcomes_ar)], levels = labs_ar,
                         ordered = T),
           desig_lab = factor(if_else(DESIGNATED == T, "Designated", "Eligible, not designated"),
                              levels = c("Designated", "Eligible, not designated"),
                              ordered = T)) %>% 
    ggplot() +
    stat_summary(aes(arrest_month_yr, value, color = desig_lab),
      fun = "mean", geom = "line"
    ) +
    geom_vline(xintercept = mdy("04-01-2018"), linetype = "dashed",
               color = "gray") +
    theme_classic() +
...
create_plots(ar_oz_adj_diff, outcomes_ar, labs_ar, "arrests")
```
```{r}
# ideal model
lmviolentarrest <- lm(violent_ellen ~ pct_white + pct_higher_ed + pct_rent +
  pct_native_hc_covered + pct_poverty + minutes_commute_avg + dec_score + SE_Flag + vacancyrate + pctunder18 +
  pctover64, ar_oz_adj_diff)

lmpropertyarrest <- lm(property_ellen ~ pct_white + pct_higher_ed + pct_rent +
  pct_native_hc_covered + pct_poverty + minutes_commute_avg + dec_score + SE_Flag + vacancyrate + pctunder18 +
  pctover64, ar_oz_adj_diff)
```
```{r}
modeldataarrest <- cbind(ar_oz_adj_diff[rownames(lmviolentarrest$model), ],
  "resid_violent" = lmviolentarrest$residuals,
  "resid_property" = lmpropertyarrest$residuals
)
modeldataarrest

modeldataarrest %>%
  ggplot() +
  stat_summary(aes(arrest_month_yr, resid_violent, color = DESIGNATED),
    fun = "mean", geom = "line"
  ) +
  theme_classic()
...
modeldataarrest %>%
  ggplot() +
  stat_summary(aes(arrest_month_yr, resid_property, color = DESIGNATED),
    fun = "mean", geom = "line"
  ) +
  theme_classic()
```
...
cp_oz_adj_diff %>%
  ggplot() +
  stat_summary(aes(cp_month_yr, violent_ellen, color = DESIGNATED),
    fun = "mean", geom = "line"
  ) +
  theme_classic()

# ideal model
lmviolentcomplaint <- lm(violent_ellen ~ pct_white + pct_higher_ed + pct_rent +
  pct_native_hc_covered + pct_poverty + minutes_commute_avg + dec_score + SE_Flag + vacancyrate + pctunder18 +
  pctover64, cp_oz_adj_diff)

lmpropertycomplaint <- lm(property_ellen ~ pct_white + pct_higher_ed + pct_rent +
  pct_native_hc_covered + pct_poverty + minutes_commute_avg + dec_score + SE_Flag + vacancyrate + pctunder18 +
  pctover64, cp_oz_adj_diff)
```
```{r}
modeldatacomplaint <- cbind(cp_oz_adj_diff[rownames(lmviolentcomplaint$model), ],
  "resid_violent" = lmviolentcomplaint$residuals,
  "resid_property" = lmpropertycomplaint$residuals
)
modeldatacomplaint

modeldatacomplaint %>%
  ggplot() +
  stat_summary(aes(cp_month_yr, resid_violent, color = DESIGNATED),
    fun = "mean", geom = "line"
  ) +
  theme_classic()
...
modeldatacomplaint %>%
  ggplot() +
  stat_summary(aes(cp_month_yr, resid_property, color = DESIGNATED),
    fun = "mean", geom = "line"
  ) +
  theme_classic()
```
...
ar_oz_plots <- ar_oz %>%
  ...
  mutate(post = factor(if_else(arrest_date > mdy("04-20-2018"), "After April 20, 2018 (Post period)", "Before April 20, 2018 (Pre period)"), levels = c("Before April 20, 2018 (Pre period)", "After April 20, 2018 (Post period)"),
                       ordered = T)) 

cp_oz_plots <- cp_oz %>%
  filter(violent_ellen == 1 & (DESIGNATED == T | (DESIGNATED == F & LIC == T))) %>%
  mutate(post = factor(if_else(cmplnt_fr_dt > mdy("04-20-2018"), "After April 20, 2018 (Post period)", "Before April 20, 2018 (Pre period)"), levels = c("Before April 20, 2018 (Pre period)", "After April 20, 2018 (Post period)"),
                       ordered = T)) 
...
ar_oz_plots %>%
  ...
  theme_classic() +
  facet_wrap(~post, nrow = 1, scales = "free_y") +
  labs(y = "Number of violent crime arrests",
...
ar_oz_plots %>%
  ...
  theme_classic() +
  facet_wrap(~post, nrow = 1, scales = "free_y") +
  labs(y = "Number of property crime arrests",
       x = "Distance from the boundary")
...
cp_oz_plots %>%
  ...
  theme_classic() +
  facet_wrap(~post, nrow = 1, scales = "free_y") +
  labs(y = "Number of violent crime complaints",
       x = "Distance from the boundary")
