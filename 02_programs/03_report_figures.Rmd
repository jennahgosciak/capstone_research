...
ar_oz_acs <- read_csv("../00_data/ar_oz_acs.csv")
...
#ideal model
lmviolentarrest <- lm(violent_ellen ~ pct_white + pct_higher_ed + pct_rent + pct_native_hc_covered + pct_poverty + minutes_commute_avg + dec_score+ SE_Flag + vacancyrate + pctunder18 + pctover64, ar_oz_acs)

#testing with just one covariate because missing data that I don't know how to deal with rn 
modeldata <- ar_oz_acs %>%  
  filter(!is.na(DESIGNATED) & !is.na(pct_white))

lmviolentarrest <- lm(violent_ellen ~ pct_white, modeldata)
summary(lmviolentarrest)

violentarrest.res <- lmviolentarrest$residuals

plot(fitted(lmviolentarrest), violentarrest.res)
plot(modeldata$distance_boundary , violentarrest.res)
