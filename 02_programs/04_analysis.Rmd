...
```{r}
# balance test data
bequiv <- map_dfr(cross2(constructs,
  c(100, seq(200, 1000, 200))),
  ~ run_baseline_checks(analysis_file, outcome = .[[1]], bandwidth = .[[2]])
)
...
if (!is.null(file_suffix)) {
  fname <- str_glue("../03_output/baseline_form_lm {Sys.Date()}{file_suffix}.csv")
} else{
  fname <- str_glue("../03_output/baseline_form_lm {Sys.Date()}.csv")
}
...
bequiv <- map_dfr(cross2(constructs,
                         seq(200, 1000, 200)),
  ~ run_baseline_checks(analysis_file %>% 
...
output_fe %>% 
  ...
  mutate(estimate_form= case_when(p.value < 0.01 ~ paste0(round(estimate, 2), "***"),
                                  ...
                                  TRUE ~ paste0(round(estimate, 2))),
         coef_form = str_c(estimate_form, "\n(", round(std.error, 2),")")) %>% 
  pivot_wider(id_cols = c("year", "order", "covs"), names_from = c("crime_type", "term", "arrest_or_complaint"),
              values_from = "coef_form") %>%
