...
outcomes_ar <- map(c("violent_ellen_winsorize_ar_", "property_ellen_winsorize_ar_"), ~ str_c(., c(1:7))) %>%
  unlist()
outcome_data_ar <- ar_oz2[, outcomes_ar]
running_var_ar <- ar_oz2[["distance_boundary"]]
covs_ar <- ar_oz2[, constructs]

outcomes_cp <- map(c("violent_ellen_winsorize_cp_", "property_ellen_winsorize_cp_"), ~ str_c(., c(1:7))) %>%
  unlist()
outcome_data_cp <- cp_oz2[, outcomes_cp]
running_var_cp <- cp_oz2[["distance_boundary"]]
covs_cp <- cp_oz2[, constructs]
```

```{r}
output <- map_dfr(
  c(1:4),
  ~ run_model_set(outcome_data_ar, running_var_ar, outcomes_ar, p = ., covs_ar)
) %>%
  bind_rows(map_dfr(
    c(1:4),
    ~ run_model_set(outcome_data_cp, running_var_cp, outcomes_cp, p = ., covs_cp)
  ))

output %>%
  separate(outcome, c("crime_type", "measurement", "winsorize", "arrest_or_complaint", "quarter")) %>%
  select(-c(measurement, winsorize)) %>%
  select(crime_type, arrest_or_complaint, term, order, covs, quarter, everything()) %>%
...
```{r}
# creating plots
walk(c(1:4), ~ rdplot(outcome_data_ar[[1]], running_var_ar[[1]],
