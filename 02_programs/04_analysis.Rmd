...
run_model_set_fe_cluster <- function(outcome_data, running_var, outcomes, p, covs, cluster) {
  ...
  return(map2_dfr(
    outcome_data_cov, outcomes,
    ~ tidy.rdrobust(.x, running_var_cov, .y, p = p, cluster = censustracts_cov)
  ))
}
```
...
bequiv %>%
  ...
  pivot_wider(id_cols = c("var"), names_from = "bandwidth", values_from = "difference") %>%
  bind_rows(n_vals) %>%
  write_csv(str_glue("../03_output/baseline_form_lm_v2 {Sys.Date()}{file_suffix}.csv"))
```
...
outcomes_cp <- map(c("violent_ellen_winsorize_cp_", "property_ellen_winsorize_cp_"), ~ str_c(., c("min4", "min3", "min1", 0, c(1:7)))) %>%
  unlist()
outcomes_cp <- outcomes_cp[1:2]
outcome_data_cp <- analysis_file[, outcomes_cp]

```

```{r}
run_model_set_cluster(outcome_data_ar, running_var, outcomes_ar, p = 1, covariates, censustracts)

theory_run_model_set_cluster(outcome_data_ar, running_var, outcomes_ar, p = 4, covariates, bandwidth = 500, censustracts)
```
...
```{r}
bandwidththeory_lin_ar <- map_dfr(
  seq(300, 600, 100),
...
bandwidththeory_lin_ar %>%
  select(outcome, bandwidth.h, term, order, covs, everything()) %>%
  filter(outcome == "violent_ellen_winsorize_ar_min4" & bandwidth.h == "[300,300]") %>%
  nrow() %>%
  magrittr::equals(6) %>%
  stopifnot()
```
...
```{r}
bandwidththeory_poly_ar <- map_dfr(
  seq(300, 600, 100),
...
```{r}
bandwidththeory_lin_cp <- map_dfr(
  seq(300, 600, 100),
  ~ theory_run_model_set_cluster(outcome_data_cp, running_var, outcomes_cp, p = 1, covariates, bandwidth = ., censustracts)
)
```
...
```{r}
bandwidththeory_poly_cp <- map_dfr(
  seq(300, 600, 100),
  ~ theory_run_model_set_cluster(outcome_data_cp, running_var, outcomes_cp, p = 4, covariates, bandwidth = ., censustracts)
)
```
...
bandwidththeory_all <- bind_rows(bandwidththeory_lin_ar, bandwidththeory_lin_cp, bandwidththeory_poly_ar, bandwidththeory_poly_cp)
...
bandwidththeory_all_formatted %>%
  ...
  pivot_wider(
    id_cols = c("quarter", "order", "covs"), names_from = c("crime_type", "term", "arrest_or_complaint", "bandwidth.h"),
    values_from = "coef_form"
  ) %>%
...
```{r}
run_model_set_fe_cluster(outcome_data_ar, running_var, outcomes_ar, p = 1, covariates, censustracts)
```
...
walk2(c(1, 4), c(400, 900), ~ generate_rd_plot(analysis_file_fe, "property_ellen_winsorize_ar_year0", "Property crime arrests, year 0", .x, c(constructs, str_c("quarter", 1:3)), "arrests", "quarter", h = .y))
```
```{r}
walk2(c(1, 4), c(400), ~ generate_rd_plot(analysis_file, "violent_ellen_winsorize_cp_0", "Violent crime complaints, quarter 0", .x, 
                                          constructs, "complaints", "quarter", h = .y))
