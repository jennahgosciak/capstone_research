...
run_model_set_cluster <- function(outcome_data, running_var, outcomes, p, covs, cluster) {
  ...
  return(map2_dfr(
    ...
    bind_rows(map2_dfr(
      outcome_data_cov, outcomes,
      ~ tidy.rdrobust(.x, running_var_cov, .y, p = p, covs = covs, cluster_cov = cluster)
    )))
}
...
bandwidththeory_poly_ar <- map_dfr(
    seq(300, 600, 100),
  ~ theory_run_model_set_cluster(outcome_data_ar, running_var, outcomes_ar, p = 4, covariates, bandwidth = ., censustracts)
)
```
...
bandwidththeory_all_formatted %>%
  ...
  pivot_wider(
    id_cols = c("quarter", "order", "covs", "bandwidth.h"), names_from = c("crime_type", "term", "arrest_or_complaint"),
    values_from = "coef_form"
  ) %>%
  select(c("quarter", "order", "covs", "bandwidth.h"), matches("Conventional"), matches("Bias"), matches("Robust")) %>%
  select(c("quarter", "order", "covs", "bandwidth.h"), matches("property"), matches("violent")) %>%
  write_csv(str_glue("../03_output/results_form_{Sys.Date()}{file_suffix}.csv"))
