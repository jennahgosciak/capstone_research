...
ar_oz2 <- ar_oz %>%
  ...
  group_by(
    GEOID_b, post, year, DESIGNATED, pct_white, pct_higher_ed, pct_rent, pct_native_hc_covered,
...
stopifnot(eeptools::isid(ar_oz2, c("GEOID_b", "post", "year")))
```
...
bws <- rdbwselect(df_rd$violent_ellen, df_rd$distance_boundary, p = 1, c = 0,
                  kernel = "tri", bwselect="mserd", 
                  covs = df_rd$post + df_rd$year)
...
```{r}
model1 <- felm(df_rd$violent_ellen[df_rd$kw > 0] ~ distance_boundary + treatment + treatment_dist | year,
