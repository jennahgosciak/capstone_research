...
```{r}
analysis_file %>% 
  mutate(missing_SE = is.na(SE_Flag),
...
theory_run_model_set_cluster <- function(outcome_data, running_var, outcomes, p, covs, bandwidth, cluster) {
  ...
  return(map2_dfr(
    ...
    bind_rows(map2_dfr(
      outcome_data_cov, outcomes,
      ~ tidy.rdrobust(.x, running_var_cov, .y, p = p, covs = covs, h = bandwidth, b = bandwidth, cluster =
                        censustracts_cov)
    )))
}
...
bandwidththeory_poly_ar <- map_dfr(
    seq(300, 600, 100),
  ~ theory_run_model_set_cluster(outcome_data_ar, running_var, outcomes_ar, p = 4, covariates,
...
```{r}
walk2(c(1, 4), c(500, 900), ~ generate_rd_plot(analysis_file_fe, "violent_ellen_winsorize_ar_year0", "Violent crime arrests, year 0", .x, c(constructs, str_c("quarter", 1:3)), "arrests", "quarter", h = .y))
