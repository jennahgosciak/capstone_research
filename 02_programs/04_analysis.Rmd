...
analysis_file_fe <- read_csv("../00_data/analysis_file_fe.csv") %>%
  mutate(
    quarter_1 = if_else(quarter2 == 1, 1, 0),
    quarter_2 = if_else(quarter2 == 2, 1, 0),
    quarter_3 = if_else(quarter2 == 3, 1, 0),
    quarter_4 = if_else(quarter2 == 4, 1, 0)
  )
...
output %>%
  ...
  select(c("quarter", "order", "covs"), matches("Robust")) %>%
  select(c("quarter", "order", "covs"), ends_with("-264"), 
...
covariates <- analysis_file_fe[, c(constructs, str_c("quarter_", 1:3))]
...
output_fe <- map_dfr(
  c(1, 4),
  ~ run_model_set_fe_cluster(outcome_data_ar, running_var, outcomes_ar, p = ., 
                             covs=covariates, cluster=censustracts)
) %>%
  bind_rows(map_dfr(
    c(1, 4),
    ~ run_model_set_fe_cluster(outcome_data_cp, running_var, outcomes_cp, p = ., 
                               covs=covariates, cluster=censustracts)
  ))

output_fe <- output_fe %>%
  separate(outcome, c("crime_type", 
                      "measurement", "winsorize", 
                      "arrest_or_complaint", "year")) %>%
  select(-c(measurement)) %>%
  select(crime_type, arrest_or_complaint, winsorize, term, 
...
analysis_file_fe %>% 
  ...
  arrange(GEOID_b, GEOID, quarter2)
```

```{r}
output_fe %>%
  filter(winsorize == "winsorize") %>%
  mutate(nobs = rowSums(select(., c(nobs.left, nobs.right))/4, na.rm = T)) %>%
  distinct(nobs, covs, crime_type)
