...
# loading acs data
acs_nyc <- get_acs(geography = "tract", variables = acs_vars,
...
acs_nyc_wide <- acs_nyc %>% 
  ...
  mutate(pct_white = white_population / population,
...
test_baseline_significance <- function(outcome, print=FALSE,
  ...
  treat_se <- vals[2,2]
  treat_pval <- vals[2,4]
  asterisk <- case_when(treat_pval < 0.01 ~ "***",
                        treat_pval < 0.05 ~ "**",
                        treat_pval < 0.1 ~ "*",
                        TRUE ~ NA_character_)
  
  data.frame("var" = outcome,
...
```
```{r}
diff_results <- map_dfr(c(names(acs_vars),
                          constructs), ~test_baseline_significance(.))
diff_results
```
```{r}
diff_results_subsample <- map_dfr(c(names(acs_vars),
                          constructs), ~test_baseline_significance(.,
                                                                   data = acs_oz_clean %>% 
